...
AddressTypeahead.prototype.bind = function (input_el, options) {
  var self = this,
      component = (0, _utils.eventMethods)({}),
      predictions = [],
      selected = {};
  ...
  var predictions_parent = options.predictions_parent ? typeof options.predictions_parent === 'string' ? focus_root.querySelector(options.predictions_parent) : options.predictions_parent : input_el.parentElement;
  ...
  function _hidePredictions() {
    ...
    predictions_wrapper.style.display = 'none';
    emitOnBlur();
    (0, _utilsDom._offClick)(document, onDocumentClick, true);
  }
  ...
  function _renderPredictions(_predictions) {
    ...
    (0, _utilsDom.toggleClass)(predictions_wrapper, '_has_predictions', predictions.length);
    ...
    for (i = 0, n = predictions.length; i < n; i++) {
      ...
      (0, _utilsDom.toggleClass)(children[i], '_custom', predictions[i].place === 'custom');
    }
  ...
  (0, _utilsDom._onInput)(input_el, (0, _utils.debounce)(onInput, options.debounce_duration));
  ...
  (0, _utilsDom._onClick)(input_el, function () {
    predictions_wrapper.style.display = '';
  });

  (0, _utilsDom._onClick)(document, onDocumentClick, true);
  ...
  (0, _utilsDom._onBlur)(input_el, function () {
    setTimeout(function () {
      if (focus_root.activeElement === input_el) return;
      if (clicked_predictions) clicked_predictions = false;else _hidePredictions();
    }, 100);
  });
